...
export async function extractUrlMetadata(pageText, url) {
    ...
    const response = await getLLM().chat.completions.create({
        ...
        messages: [
            {
                role: "system",
                content: "You extract structured metadata from web page content. Return ONLY valid JSON, no markdown fences.",
            },
            {
                role: "user",
...
export async function extractPaperMetadata(rawText) {
    ...
    const response = await getLLM().chat.completions.create({
        ...
        messages: [
            {
                role: "system",
                content: "You extract structured metadata from academic paper text. Return ONLY valid JSON, no markdown fences.",
            },
            {
                role: "user",
...
export async function findOrCreateProfile(discordUserId, discordDisplayName) {
...
export async function linkProfile(discordUserId, profileName) {
    const supabase = getSupabase();
 ...
 */
export async function checkDuplicateUrl(url, profileId) {
    const supabase = getSupabase();
...
export async function getCycleStats() {
    ...
    const { data: cycle } = await supabase
        ...
        .single();
    if (!cycle)
        return { totalItems: 0, contributorCount: 0, cycleNumber: 0 };
    ...
    const { data: contributors } = await supabase
        ...
        .select("profile_id")
        .eq("cycle_id", cycle.id);
    const uniqueContributors = new Set(contributors?.map((c) => c.profile_id)).size;
    return {
        totalItems: count || 0,
...
// --- Chat with Funnel ---
export async function chatWithFunnel(question, dmUserName) {
    const supabase = getSupabase();
 ...
 */
export async function embedSubmission(submissionId, title, body) {
    try {
...
async function llmExtractConcepts(title, body, existingConcepts) {
    ...
    const response = await getLLM().chat.completions.create({
        ...
        messages: [
            {
                role: "system",
                content: "You extract hierarchical research concepts from academic content. Return ONLY valid JSON, no markdown fences.",
            },
            {
                role: "user",
    ...
    try {
        ...
        const concepts = Array.isArray(parsed.concepts)
            ? parsed.concepts
                .map((c) => {
                if (typeof c === "string") {
                    return { label: c.toLowerCase().trim(), level: "specific" };
                }
                return {
                    label: String(c.label || "").toLowerCase().trim(),
                    level: c.level === "broad" ? "broad" : "specific",
                };
            })
...
export async function fetchAndExtractUrl(url) {
    const res = await fetch(url, {
        headers: {
            "User-Agent": "Mozilla/5.0 (compatible; KnowledgeFunnel/1.0; +https://knowledge-funnel.vercel.app)",
