...
export async function handleSubmitLink(interaction) {
    ...
    try {
        ...
        const profileId = await findOrCreateProfile(interaction.user.id, displayName);
        // Check if this person already submitted this exact URL
        const duplicate = await checkDuplicateUrl(url, profileId);
        if (duplicate) {
            await interaction.editReply(`You've already submitted this link: **${duplicate.title}**`);
            return;
        }
        // Fetch and extract metadata (graceful fallback if site blocks us)
        let metadata = { title: "", authors: "", year: null, abstract: "", keywords: [] };
        let fetchWarning = "";
        try {
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription(metadata.title ? `**${metadata.title}**\n${url}` : url)
        ...
        await interaction.editReply("Something went wrong processing that link. Check the URL and try again.");
    }
}
export async function handleSubmitNote(interaction) {
    ...
    const type = (interaction.options.getString("type") || "note");
    try {
        const displayName = interaction.member && "displayName" in interaction.member
            ? interaction.member.displayName
            : interaction.user.displayName || interaction.user.username;
...
export async function handleFunnelStatus(interaction) {
    await interaction.deferReply();
    try {
...
export async function handleMySubmissions(interaction) {
    await interaction.deferReply();
    try {
        const displayName = interaction.member && "displayName" in interaction.member
            ? interaction.member.displayName
        ...
        if (!submissions || submissions.length === 0) {
            await interaction.editReply("You haven't submitted anything this cycle yet. Use `/submit-link` or `/submit-note` to get started!");
            return;
        }
        const lines = submissions.map((s) => {
            const icon = s.content_type === "paper"
                ? "📄"
                : s.content_type === "link"
                    ? "🔗"
                    : s.content_type === "idea"
...
export async function handleMention(message, isDM = false) {
...
export async function handlePdfAttachment(message, attachment) {
    const channelId = process.env.DISCORD_CHANNEL_ID;
    const isDM = message.channel.type === ChannelType.DM;
    if (!isDM && channelId && message.channelId !== channelId)
        return;
    const processingMsg = await message.reply("Processing your PDF...");
    try {
        const displayName = message.member?.displayName || message.author.displayName || message.author.username;
        const profileId = await findOrCreateProfile(message.author.id, displayName);
        // Download the PDF
        ...
        if (userComment)
            parts.push(`Comment: ${userComment}`);
        if (abstract)
            parts.push(abstract);
        ...
        await processingMsg.delete().catch(() => { });
        await message.reply("Something went wrong processing that PDF. Please try again.");
    }
}
